...
class userEditor extends CommandLogger implements UserController
{        
    /**
     * @return userEditor
     */
    ...
    public function handleRequest(HttpRequest $request)
    {
        ...
        if ($this->isDisplayView($mav)) {
            ...
            if (in_array($this->getForm()->{$this->getActionMethod()}('action'), array(self::ACTION_UPDATE))) {
                Singleton::getInstance('HTMLMetaManager')->appendJavaScript('/i/user-editor.js');
            }            
        }
    ...
    {
        return CommonUtils::makeFrontUrl('userList');
    }
}