...
class baseAjaxUserRegisterCode extends baseAjax
{
    ...
    {        
        ...
        if ($userExists instanceof User) {
             ...
             */
            try {
                $codeExists = Confirm::dao()->getByLogic(Expression::andBlock(Expression::eq('type_id', ConfirmType::TYPE_REGISTRATION_PHONE), Expression::eq('phone', $form->getValue('phone'))));
            } catch(ObjectNotFoundException $e) { /* nothin here */ }
            ...
            if ($codeExists instanceof Confirm) {
                if (Timestamp::compare($codeExists->getCreatedTime(), Timestamp::create("-1 minute")) == -1) {
                    ...
                    SmsUtils::send("7{$form->getValue('phone')}", sprintf("Код подтверждения для регистрации: %04d", $codeExists->getCode()));
